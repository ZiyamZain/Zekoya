...
const CategoryOfferList = () => {
  ...
  // Pagination – backend first, else derive
  const itemsPerPage = 10;
  const calculatedTotalPages = totalPages || Math.max(1, Math.ceil((categoryOffers?.length || 0) / itemsPerPage));
  const displayedOffers = totalPages ? categoryOffers : (categoryOffers || []).slice((page - 1) * itemsPerPage, page * itemsPerPage);

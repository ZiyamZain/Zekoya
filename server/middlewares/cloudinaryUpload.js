...
const createCloudinaryStorage = (options) => new CloudinaryStorage({
  cloudinary,
    ...
    if (req.files && req.files.length > 0) {
      const cleanupPromises = req.files.map((file) => {
        if (file.path && file.filename) { // file.path is the URL, file.filename is the public_id
          return cloudinary.uploader.destroy(file.filename).catch((e) => {
            console.error('Cleanup failed for:', file.filename, e);
          });
        }

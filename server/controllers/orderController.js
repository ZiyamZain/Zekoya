...
export const cancelOrderItem = async (req, res) => {
  try {
    ...
    if (order.isPaid && order.paymentMethod !== 'Cash on Delivery') {
      ...
      if (user) {
        ...
        order.itemsPrice = Number(
          (order.itemsPrice - (orderItem.price * orderItem.quantity)).toFixed(2),
        );
        order.taxPrice = Number((order.taxPrice - taxRefund).toFixed(2));
        order.totalPrice = Number(
          (order.itemsPrice + order.taxPrice + order.shippingPrice).toFixed(2),
        );
    ...
    const newItemsTotal = activeItems.reduce(
      (total, item) => total + item.price * item.quantity,
      0,
    );

    // Check if we need to update shipping price based on the new total
    // Define your free shipping threshold
    const FREE_SHIPPING_THRESHOLD = 1000;
    // Define your standard shipping cost
    const SHIPPING_COST = 100;
    ...
    order.totalPrice = newItemsTotal + order.taxPrice + newShippingPrice
